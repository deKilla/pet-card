...
public interface MedicationRepository extends PagingAndSortingRepository<Medication, Long> {
  ...
  //selects all medication by one pet (id)
  @Query(value = "select m.* from medication m join pet_medication pm on m.id = pm.medication_id join pet p on pm.pet_id = p.id where p.id = ?1 order by pm.id", nativeQuery = true)
